...
void CGfxSplitterWnd::OnDrawSplitter(CDC* pDC, ESplitType nType, const CRect& rectArg)
{
	...
	case splitBox:
		...
		rect.InflateRect(-CX_BORDER, -CY_BORDER);
		DrawSkinImageSection( m_hSkin ,"SplitterBox" ,pDC->m_hDC, &rect );
		break;
	default:
		DrawSkinImageRect( m_hSkin ,"Backbround" ,pDC->m_hDC, &rect );
		break;
	}
}
...
//	return CSplitterWnd::OnEraseBkgnd(pDC);
	...
	DrawSkinImageRect( m_hSkin ,"Backbround" ,pDC->m_hDC, &rectClient ); 
...
LRESULT CGfxSplitterWnd::OnSkinDataChanged(WPARAM wParam, LPARAM lParam)
{
	if( m_hSkin ) CloseSkinData( m_hSkin );

	m_hSkin = OpenSkinData("SkinGfxSplitter1");
	if( m_hSkin )
	{
		RedrawWindow();
	}
