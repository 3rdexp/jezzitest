...
#define _ID_GFX_REMOVEFOLDER	"Remove folder"
#define _ID_GFX_RENAMEFOLDER	"Rename folder"
//
	...
	if (iLastSel >= iFirstItem && irc.bottom > rc.bottom && irc.top < rc.top)
	{
		...
		if (IsSmallIconView())
		{
			if (ima)
			{
				...
				DrawSkinImageSection( m_hSkin ,"PressedItem" ,dc.m_hDC, &rcBck ); 
	...
	if (!GetFolderChild())
	{
		...
		if (f > 0)
		{
			if (bUpPressed)
			{
				...
				DrawSkinImageSection( m_hSkin ,"PressedUpArrowButton" ,pDC->m_hDC, &rcUpArrow ); 
			}
			else
			{
				//pDC->DrawFrameControl(rcUpArrow, DFC_SCROLL, DFCS_SCROLLUP);
				DrawSkinImageSection( m_hSkin ,"UpArrowButton" ,pDC->m_hDC, &rcUpArrow ); 
			}
			bUpArrow = true;
		...
		if (l < GetItemCount() - 1)
		{
			if (bDownPressed)
			{
				...
				DrawSkinImageSection( m_hSkin ,"PressedDownArrowButton" ,pDC->m_hDC, &rcDownArrow ); 
			}
			else
			{
				//pDC->DrawFrameControl(rcDownArrow, DFC_SCROLL, DFCS_SCROLLDOWN);
				DrawSkinImageSection( m_hSkin ,"DownArrowButton" ,pDC->m_hDC, &rcDownArrow ); 
			}
			bDownArrow = true;
...
void CGfxOutBarCtrl::DrawFolder(CDC * pDC, const int iIdx, CRect rect, const bool bSelected)
{
	...
	if (!bSelected)
	{
		if( m_hSkin )
		{
			DrawSkinImageSection( m_hSkin, "NormalFolder" , pDC->m_hDC , &rect );
		}
		else
		{
	...
	else
	{
		if( m_hSkin )
		{
			DrawSkinImageSection( m_hSkin, "SelectedFolder" , pDC->m_hDC , &rect );
		}
		else
		{
			CPen pn1(PS_SOLID, 1, crBackGroundColor1);
...
void CGfxOutBarCtrl::HighlightFolder(const int index)
{
	...
	if (iLastFolderHighlighted >= 0)
	{
		...
		if (GetFolderRect(iLastFolderHighlighted, rc))
		{
			...
			{
				DrawFolder( &dc, iLastFolderHighlighted, rc, false );
			}
		}
	}
	...
	if (iLastFolderHighlighted >= 0)
	{
		...
		if (GetFolderRect(iLastFolderHighlighted, rc))
		{
			...
			{
				CBarFolder * pbf = (CBarFolder *) arFolder.GetAt(iLastFolderHighlighted);
				DrawSkinImageSection( m_hSkin ,"HoverFolder" ,dc.m_hDC, &rc ); 
				rc.InflateRect(-1,-1);
				int obk = dc.SetBkMode(TRANSPARENT);
				CFont * of = dc.SelectObject(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
				dc.DrawText(CString(pbf->cName), rc, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
				dc.SetBkMode(obk);
				dc.SelectObject(of);
			}
		}
...
void CGfxOutBarCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (ht == htDownScroll)
	{
		...
		if (::GetCapture() == NULL)
		{
			...
			DrawSkinImageSection( m_hSkin ,"PressedDownArrowButton" ,dc.m_hDC, &rcDownArrow ); 
				...
				case WM_MOUSEMOVE:
					{
						...
						else
						{
							if (bHigh == true)
							{
								...
								DrawSkinImageSection( m_hSkin ,"DownArrowButton" ,dc.m_hDC, &rcDownArrow ); 
								bHigh = false;
								bDownArrow = false;
								bDownPressed = false;
							}
	...
	if (ht == htUpScroll)
	{
		...
		if (::GetCapture() == NULL)
		{
			...
			DrawSkinImageSection( m_hSkin ,"PressedUpArrowButton" ,dc.m_hDC, &rcUpArrow ); 
				...
				case WM_MOUSEMOVE:
					{
						...
						else
						{
							if (bHigh == true)
							{
								...
								DrawSkinImageSection( m_hSkin ,"UpArrowButton" ,dc.m_hDC, &rcUpArrow ); 
								bHigh = false;
								bUpArrow = false;
								bUpPressed = false;
							}
...
void CGfxOutBarCtrl::RemoveFolder(const int index)
{
	...
	{
		int iRet = MessageBox("Are you sure to delete the folder(Y/N)?", "Information", MB_YESNO);
		if(iRet == IDYES)
		{			
			bSureFlag = true;
		}
...
void CGfxOutBarCtrl::DrawItem(CDC * pDC, const int iFolder, CRect rc, const int index, const bool bOnlyImage)
{
	...
	if (dwFlags&fSelHighlight && iLastSel == index && iLastSel >= 0)
	{
		...
		DrawSkinImageSection( m_hSkin ,"HighlightItem" ,pDC->m_hDC, &rc ); 
...
void CGfxOutBarCtrl::HighlightItem(const int index, const bool bPressed)
{
	...
	if (iLastItemHighlighted >= 0 && IsValidItem(iLastItemHighlighted))
	{
		...
		DrawSkinImageRect( m_hSkin ,"BackImg" ,dc.m_hDC, &rc ); 
		rc.DeflateRect(1,1);
		DrawItem( &dc , iSelFolder , rc , iLastItemHighlighted , true );
	}
	...
	if (iLastItemHighlighted >= 0 && IsValidItem(iLastItemHighlighted))
	{
		...
		if (bPressed)
		{
			...
			DrawSkinImageSection( m_hSkin ,"PressedItem" ,dc.m_hDC, &rc ); 

		}
		else
		{
			//dc.Draw3dRect(rc, cr3dFace, crDkShadowBorder);
			DrawSkinImageSection( m_hSkin ,"HighlightItem" ,dc.m_hDC, &rc ); 
		}
		rc.DeflateRect(1,1);
		DrawItem( &dc , iSelFolder , rc , iLastItemHighlighted , true );
...
void CGfxOutBarCtrl::AnimateFolderScroll(const int iFrom, const int iTo)
{
	...
	if (iTo > iFrom)
	{
		...
		{
			DrawSkinImageRect( m_hSkin ,"BackImg" ,pDC->m_hDC, &rc1 ); 
		}
...
LRESULT CGfxOutBarCtrl::OnSkinDataChanged(WPARAM wParam, LPARAM lParam)
{
