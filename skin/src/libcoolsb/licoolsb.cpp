...
HDC WINAPI LoadSkinRes(RECT rc)
{
	...
	hSkinBmp = CreateCompatibleBitmap(hdc, nWidth, nHeight);
	hSkinBmpOld = SelectObject(hdcSkin, hSkinBmp);
	ReleaseDC(0, hdc);
	return hdcSkin;
...
void WINAPI ReleaseSkinRes()
{
	
	if( hdcSkin && hSkinBmpOld )
		SelectObject( hdcSkin, hSkinBmpOld );
