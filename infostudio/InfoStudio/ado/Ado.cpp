...
SYSTEMTIME vartodate(const _variant_t& var)
{
	SYSTEMTIME value;	
	switch (var.vt) 
	{
	case VT_DATE:
		{
			VariantTimeToSystemTime(var, &value);
			//time_t get_time = time(NULL);;
			//get_time = var.date;
			//TimetToSystemTime(get_time, &value);			
		}
		break;
	case VT_EMPTY:
	case VT_NULL:		
...
{
	CString strConnect = _T("Provider=Microsoft.Jet.OLEDB.4.0; Persist Security Info=False;") ;
	if (pass != _T("")) 
	{
		strConnect += _T("Jet OLEDB:Database Password=") + pass + _T(";");
	}
	strConnect += _T("Data Source= ") + dbpath;
	return Open(LPCTSTR(strConnect), lOptions);
}
