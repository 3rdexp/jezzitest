...
CAdoCommand::CAdoCommand()
{
	...
	if (m_pCommand == NULL)
	{
		TRACE("Command 对象创建失败! 请确认是否初始化了Com环境.");
	}
	#endif
...
CAdoParameter::CAdoParameter()
{
	...
	{
		TRACE("Parameter 对象创建失败! 请确认是否初始化了Com环境.");
	}
	#endif
...
BOOL CAdoParameter::SetValue(const SYSTEMTIME &value)
{
	_variant_t var;	
	var.vt = VT_DATE;	
	SystemTimeToVariantTime((LPSYSTEMTIME)&value, &var.date);
	try
	{
		if (m_pParameter->Size == 0)
		{
...
BOOL CAdoParameter::GetValue(SYSTEMTIME &value)
{
	try
	{
