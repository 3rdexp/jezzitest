...
inline std::wstring string2w(const std::string & text)
{
    // utf16 => ansi
    const int len = MultiByteToWideChar(CP_ACP, 0, text.c_str(), text.size()
        , 0, 0);
    wchar_t * w = new wchar_t[len + 1];
    int cd = MultiByteToWideChar(CP_ACP, 0, text.c_str(), text.size(), w, len);
    ASSERT(cd == len);
