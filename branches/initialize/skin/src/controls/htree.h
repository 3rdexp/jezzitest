...
namespace SkinDefault
{
	...
	class ATL_NO_VTABLE TreeViewConfig : public CComTearOffObjectBase<TreeViewDraw>
		, public ISkinConfig  
	{
	public:
		typedef TreeViewConfig thisType;
		...
		{
			SKINCONFIG_BEGIN_PART(TVP_TREEITEM)
		...
		{
			SKINCONFIG_BEGIN_PART(LVP_TREEOUTBORDER)
			...
			SKINCONFIG_BEGIN_PART(LVP_TREEINBORDER)
	...
	class ATL_NO_VTABLE TreeViewDraw : public CComObjectRoot
		, public SkinDrawImpl<TreeViewDraw, SKINCTL_TREEVIEW>
	{
	public:
		typedef TreeViewDraw thisType;
		BEGIN_COM_MAP(thisType)
			COM_INTERFACE_ENTRY_IID(IID_ISkinDraw, ISkinDraw)
			COM_INTERFACE_ENTRY_CACHED_TEAR_OFF(IID_ISkinConfig, TreeViewConfig, _spCustomConfig.p)
		END_COM_MAP()
