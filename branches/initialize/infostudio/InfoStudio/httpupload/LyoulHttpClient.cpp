/*!
 * \file	LyoulHttpClient.cpp
 * \brief	Implementations of lyoul's HTTP client classes.
 * \author	Hyung-Lyoul CHO
 * \since	2004.04.12
 * \version	$LastChangedRevision: 51 $
 *			$LastChangedDate: 2004-08-09 22:31:33 +0900 (Mon, 09 Aug 2004) $
 * 
 * <dl compact>
 ...
 * </dl>
 * This file contains implementations of lyoul's HTTP client classes.
 * \n\n\n
...
LPCSTR CHttpToolA::szDefUsrAgent = "Lyoul HTTP Client Class" ;
...
LPCSTR CHttpToolA::szDefBoundary = "----LYOUL-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCSTR CHttpToolA::szDefUploadContType = "multipart/form-data; boundary=" "----LYOUL-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCSTR CHttpToolA::szNULL = "NULL" ;
LPCSTR CHttpToolA::szEmptyString = "" ;
LPCSTR CHttpToolA::szColonSlashSlash = "://" ;


LPCWSTR CHttpToolW::szDefUsrAgent = L"Lyoul HTTP Client Class" ;
...
LPCWSTR CHttpToolW::szDefBoundary = L"----LYOUL-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCWSTR CHttpToolW::szDefUploadContType = L"multipart/form-data; boundary=" L"----LYOUL-FB3B405B7EAE495aB0C0295C54D4E096-" ;
	...
	throw (Exception &)
{
	...
	_ASSERTE ( (CodePage != CP_UTF8) && (CodePage != CP_UTF7) ) ;
...
HINTERNET CHttpToolA::OpenConnection (HINTERNET hInternet, LPCSTR szServerAddr, INTERNET_PORT nPort
	...
	throw (Exception &)
{
	_ASSERTE ( hInternet != NULL ) ;
	_ASSERTE ( szServerAddr != NULL ) ;
	_ASSERTE ( ::strlen (szServerAddr) != 0 ) ;
	...
	throw (Exception &)
{
	...
	_ASSERTE ( szObjectName != NULL ) ;
	_ASSERTE ( ::strlen (szObjectName) != 0 ) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		ThrowException ("Error en CHttpToolA::AddHeader") ;
	}

	PSTR		szHeader = (PSTR) ::malloc (sizeof (CHAR) * cbHeader) ;
	if ( szHeader == NULL )
		ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		ThrowException ("Error en CHttpToolA::SendRequest") ;
	}
	...
	if ( !::HttpSendRequestA (
			...
			, cchPosted					// The length of the posted data
		) )
	...
	throw (Exception &)
{
	...
	try {
		...
		ThrowException ("Error en CHttpToolA::GetMimeType") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		ThrowException ("Error en CHttpToolA::GetHeader") ;
	}

	PSTR		szHeader = (PSTR) ::malloc (cbBuff);
	if ( szHeader == NULL )
		ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
	::strcpy (szHeader, szName) ;
...
// Generates a new upload boundary. If an error occurred, NULL is returned
// The returned string must be freed by using the ::free () function.
	...
	throw (Exception &)
{
	_ASSERTE ( hInternet != NULL ) ;
	_ASSERTE ( szServerAddr != NULL ) ;
	_ASSERTE ( ::wcslen (szServerAddr) != 0 ) ;
	...
	throw (Exception &)
{
	_ASSERTE ( hConnection != NULL ) ;
	_ASSERTE ( szObjectName != NULL ) ;
	_ASSERTE ( ::wcslen (szObjectName) != 0 ) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		SAFEFREE (szPostedA) ;
		ThrowException ("Error en CHttpToolW::SendRequest") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		ThrowException ("Error en CHttpToolW::GetHeader") ;
	}

	PWSTR		szHeader = (PWSTR) ::malloc (cbBuff);
	if ( szHeader == NULL )
		ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
	...
	throw ()
{
	...
	// If an overflow exception or a memory allocation failure is occurred,
	// The memory pointed by the key name pointer are leaked.
	MapSizeType				cKeys = m_map.size () ;
	size_t					cbRequired = 0 ;
	PCSZ *					arrKeys = NULL ;
	...
	try {
		...
		arrKeys = (PCSZ *) ::malloc (cbRequired) ;
	} 
	catch (...) {
		arrKeys = NULL ;
	}
	...
	throw ()
{
	...
	MapSizeType	cKeys = m_map.count (szName) ;
	...
	throw ()
{
	...
	try {
		...
		cKeys = 1024 * 32;	// This statement is never executed.
	}
	return cKeys;
}
	...
	throw (Exception &)
{
	...
	if ( szValue ) {
		...
		try {
			...
			HttpTool::ThrowException ("Error en CHttpClientMapT<HttpTool>::Set") ;
		}

		newValue.szValue = (PCSZ) ::malloc (cbValue) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::AnsiEncodeLen") ;
	}
	return dwLen;
}
 ...
 * It just returns the length of the input string.
 *
 * \param szEncoded		[in] A string which is decoded.
 * \param CodePage		[in] Ignored.
 * \return				The number of bytes required. (Not including a terminating NULL character)
 * \throw				Throws a httpclientexception if an error occurred.
 */
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::AnsiDecodeLen") ;
	}
	return dwLen;
}
 ...
 * \return				The number of characters required. (Not including a terminating NULL character)
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool>
	...
	throw (typename HttpTool::Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en _UrlEncodeLen") ;
	}
	return cchEncoded;
}
...
void CHttpEncoderA::_AnsiCharToUtf8Char (PSTR szUtf8Char, PCSTR szAnsiChar, UINT CodePage)
	throw (Exception &)
{
	_ASSERTE ( szUtf8Char != NULL && szAnsiChar != NULL ) ;
...
void CHttpEncoderA::_Utf8CharToAnsiChar (PSTR szAnsiChar, PCSTR szUtf8Char, UINT CodePage)
	throw (Exception &)
{
	_ASSERTE ( szAnsiChar != NULL && szUtf8Char != NULL ) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::Utf8EncodeLen") ;
	}
	return cchEncoded;
}
...
/*!
 * This method returns the number of bytes required to decode an UTF-8 string using Ansi encoding.
 * The returned value does not include a terminating NULL character.
 * The detailed infomation about the Code-Page Identifiers is described in the MSDN documentation.
 *
 * \param szEncoded		[in] A string which is decoded.
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::Utf8DecodeLen") ;
	}
	return cchDecoded;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::UrlEncodeLenA") ;
	}
	return cchEncoded;
}
 ...
 * The returned value does not include a terminating NULL character.
 * This method does not support the URL-encoded string containing unicode character. (\%u or \%x)
 * The detailed infomation about the Code-Page Identifiers is described in the MSDN documentation.
 *
 * \param szEncoded		[in] A string which is decoded.
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::UrlDecodeLenA") ;
	}
	return cchDecoded;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolA::ThrowException ("Error en CHttpEncoderA::UrlDecodeLenW") ;
	}
	return cchDecoded;
}

/*!
 * This method decodes an URL-encoded string to make an Ansi string.
 * This method does not support the URL-encoded string containing unicode character. (\%u or \%x)
 * The detailed infomation about the Code-Page Identifiers is described in the MSDN documentation.
 *
 * \param szBuff		[out] A buffer to save the decoded string. The buffer can not be NULL.
...
/*!
 * This method decodes an URL-encoded string to make an Unicode string.
 * This method does not support the URL-encoded string containing unicode character. (\%u or \%x)
 * The detailed infomation about the Code-Page Identifiers is described in the MSDN documentation.
 *
 * \param szBuff		[out] A buffer to save the decoded string. The buffer can not be NULL.
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::AnsiEncodeLen");
	}
	return cchAnsiLen;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::AnsiEncode") ;
	}

	if ( 0 == ::WideCharToMultiByte (CodePage, 0, szStr, -1, szBuff, cbAnsiLen, NULL, NULL) )
		CHttpToolW::ThrowException (HTTPCLIENT_ERR_WIDECHARTOMULTIBYTE_FAILED, ::GetLastError ()) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::AnsiDecodeLen") ;
	}
	return cchUnicodeLen;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::AnsiDecode") ;
	}
	if ( 0 == ::MultiByteToWideChar (CodePage, 0, szEncoded, -1, szBuff, cchUnicode) )
		CHttpToolW::ThrowException (HTTPCLIENT_ERR_MULTIBYTETOWIDECHAR_FAILED, ::GetLastError ()) ;
...
/*!
 * This method returns the number of bytes required to encode an Unicode string using UTF-8 encoding.
 * The returned value does not include a terminating NULL character.
 *
 * \param szStr			[in] A string which is encoded.
 * \param CodePage		[in] Ignored.
	...
	throw (Exception &)
{
	...
	try  {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::Utf8EncodeLen") ;
	}
	return cchRequired;
}
	...
	throw (Exception &)
{
	...
	try  {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::Utf8Encode") ;
	}
	if ( 0 == ::WideCharToMultiByte (CP_UTF8, 0, szStr, -1, szBuff, cchRequired, NULL, NULL) )
		CHttpToolW::ThrowException (HTTPCLIENT_ERR_WIDECHARTOMULTIBYTE_FAILED, ::GetLastError ()) ;
...
/*!
 * This method returns the number of unicode characters required to decode an UTF-8 string using Unicode encoding.
 * The returned value does not include a terminating NULL character.
 *
 * \param szEncoded		[in] A string which is decoded.
 * \param CodePage		[in] Ignored.
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::Utf8DecodeLen") ;
	}
	return cchRequired;
}

/*!
 * This method decodes an UTF-8 string using Unicode encoding.
 *
 * \param szBuff		[out] A buffer to save the decoded string. The buffer can not be NULL.
 * \param szEncoded		[in] A string which is decoded.
 * \param CodePage		[in] Ignored.
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::Utf8Decode") ;
	}

	if ( 0 == ::MultiByteToWideChar (CP_UTF8, 0, szEncoded, -1, szBuff, cchRequired) )
		CHttpToolW::ThrowException (HTTPCLIENT_ERR_MULTIBYTETOWIDECHAR_FAILED, ::GetLastError ()) ;	
	return szBuff ;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::UrlEncodeLenA") ;
	}
	return cchEncoded;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::UrlDecodeLenA") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		CHttpToolW::ThrowException ("Error en CHttpEncoderW::UrlDecodeLenW") ;
	}
	...
	try {
		try {
			...
			while ( true ) {
				...
				// Copy the header name
				if ( NULL == (szCopiedName = (PSZ) ::malloc (cbName)) )
					HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
			...
			HttpTool::ThrowException ("Error en CHttpResponseT") ;
		}
	} catch (Exception &) {
...
/*!
 * This method returns the number of headers of which name is the szName.
 *
 * \param szName		[in] The case-insensitive name of the header. NULL is not allowed.
 * \return				The number of headers of which name is the szName.
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool>
 ...
 * If the header has multiple values, you can specify the zero-based index for a specific value.
 * If the header is not found or the index is out of range, it will return NULL.
 ...
 *
 * \param szName		[in] The case-insensitive name of the header. NULL is not allowed.
 * \param nIdx			[in] The zero-based index for the header which has multiple values.
 * \return				The requested header.
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool>
typename CHttpResponseT<HttpTool>::PCSZ
CHttpResponseT<HttpTool>::GetHeader (PCSZ szName, DWORD nIdx)
	throw (Exception &)
{
	_ASSERTE ( szName != NULL ) ;
...
/*!
 * This method returns the HTTP status code returned by a HTTP server.
 *
 * \return				The HTTP status code returned by a HTTP server.
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool>
...
/*!
 * This method returns the HTTP status text returned by a HTTP server.
...
/*!
 * This method retrieves the length of the content returned by a HTTP server.
 ...
 *
 * \param cbContLen		[out] A reference variable that you want to receive the length of the content.
 * \return				Returns TRUE if the length of the content is returned, othrewise returns FALSE.
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool>
...
/*!
 * This method reads the content (the data stream except headers) returned by a HTTP server.
 *
 * \param pbyBuff		[in, out] A pointer to a buffer.
 * \param cbBuff		[in] the size of the buffer.
...
/*!
 * This method returns the actual total number of bytes of the request.
 * The returned size includes the size of boundaries, parameter names, and other elements.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The number of bytes which is actually sent to the HTTP server
 */
template <typename HttpTool>
...
/*!
 * This method returns the actual number of posted bytes of the request.
 * The returned size includes the size of boundaries, parameter names, and other elements.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The number of bytes which has actually been sent to the HTTP server
 */
template <typename HttpTool>
...
/*!
 * This method returns the total number of bytes of the request.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The total number of bytes of the request
 */
template <typename HttpTool>
...
/*!
 * This method returns the number of posted bytes of the request.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The number of posted bytes of the request
 */
template <typename HttpTool>
...
/*!
 * This method returns the total number of parameters of CHttpClient object.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 ...
 * The count includes the current parameter which is been sending to the server.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The number of parameters which have been sent to the HTTP server
 */
template <typename HttpTool>
 ...
 * The file parameter contains the file path to upload.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 ...
 * If the current parameter is a file parameter, the count includes the current parameter.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
...
/*!
 * This method returns the name of the current parameter.
 * The current parameter is the parameter which is been sending to the server.
 * It always returns a null-terminated string. The returned string is owned by
 * the CHttpPostStat. So you can not free it.
 * If the system failed to allocate memory for the name of the current parameter,
...
/*!
 * This method returns the file path of the current parameter.
 * The current parameter is the parameter which is been sending to the server.
 * It always returns a null-terminated string. The returned string is owned by
 * the CHttpPostStat. So you can not free it.
 * If the current parameter is not a file parameter or the system failed to allocate
 * memory for the file path of the current parameter, it will return "NULL".
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The file path of the current parameter
 */
template <typename HttpTool>
typename CHttpPostStatT<HttpTool>::PCSZ
...
/*!
 * This method returns the total number of bytes of the current parameter.
 * The current parameter is the parameter which is been sending to the server.
 * If the IsActive method returns FALSE, you shouldn't call this method.
 *
 * \return			The total number of bytes of the current parameter
 */
template <typename HttpTool>
 ...
 * The current parameter is the parameter which is been sending to the server.
 *
 * \return			Returns TRUE if the current parameter is a file parameter
 */
template <typename HttpTool>
...
/*!
 * This method returns TRUE if the current parameter is completly posted.
 * The current parameter is the parameter which is been sending to the server.
 *
 * \return			Returns TRUE if the current parameter is completly posted
 */
template <typename HttpTool>
	...
	throw (Exception &)
{
	...
	try {
		size_t	cbActualPosted = m_cbActualPosted ;

		_ASSERTE ( (cbActualPosted + nBytes) <= m_cbActualTotal ) ;
		cbActualPosted += nBytes ;
	} 
	catch (...) {
		HttpTool::ThrowException ("Error en CHttpPostStatT<HttpTool>::_TestAddActualPostedBytes") ;
	}
}
	...
	throw (Exception &)
{
	...
	try {
		DWORD	cParamPosted = m_cParamPosted ;
		_ASSERTE ( (cParamPosted + 1) <= m_cParam ) ;
		cParamPosted++ ;

		if ( bIsFile ) {
			DWORD		cFilePosted = m_cFilePosted;
			_ASSERTE ( (cFilePosted + 1) <= m_cFile ) ;
			cFilePosted++ ;
		}
	} 
	catch (...) {
		HttpTool::ThrowException ("Error en CHttpPostStatT<HttpTool>::_TestStartNewEntry") ;
	}
}
	...
	throw (Exception &)
{
	...
	try {
		...
		size_t		cbPosted = m_cbPosted ;
		size_t		cbCurrParamPosted = m_cbCurrParamPosted ;
		...
		HttpTool::ThrowException ("Error en CHttpPostStatT<HttpTool>::_TestAddPostedBytes") ;
	}
}
...
/*!
 * This method analyzes a URL and saves information of its component parts.
	...
	throw (Exception &)
{
	...
	// Checks overflow
	try {
		DWORD		dw = HttpTool::StringLen (szUrl) ;
	} 
	catch (...) {
		HttpTool::ThrowException ("Error en CHttpUrlAnalyzerT<HttpTool>::Analyze") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_String2Ansi") ;
	}

	PSTR		szBuff = (PSTR) ::malloc (cbRequired);
	if ( szBuff == NULL )
		HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_UrlEncodeA") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_UrlEncode");
	}

	PSZ			szBuff = (PSZ) ::malloc (cbRequired);
	if ( szBuff == NULL )
		HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_Utf8Encode") ;
	}
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_AllocNCopyIfNewString") ;
	}

	PSZ		szNewString = (PSZ) ::malloc (cbRequired) ;
	if ( szNewString == NULL )
		HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
...
/*!
 * When the CHttpClient class uploads files using RequestUpload or BeginUpload method,
 * it does not throw an exception if it fails to open a file which is going to be uploaded.
 * If this property is TRUE, it will throw a httpclientexception of which error code
 * is HTTPCLIENT_ERR_OPENFILE_FAILED when it fails to open a file. The default is FALSE.
 *
...
/*!
 * If this property is TRUE, the CHttpClient class will send all parameters using UTF-8
 * encoding. Otherwise it will send all parameters using ANSI encoding.
 * The default is FALSE.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
...
/*!
 * This method sets An ANSI code page which is used to interpret non-UNICODE strings.
 * If you want to interact with another language web server and the server does not use UNICODE (UTF-8 encoding),
 * you have to use the web server's ANSI code page to get correct result.
 * The default is CP_ACP (system's ANSI code page).
 * The detailed infomation about the Code-Page Identifiers is described in the MSDN documentation.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 * You also can not set the code page for UNICODE encoding.
 * 
	...
	throw ()
{
	...
	_ASSERTE ( nAnsiCodePage != CP_UTF7 && nAnsiCodePage != CP_UTF8 ) ;
...
/*!
 * This method returns the current ANSI code page.
 *
...
/*!
 * This method erases all HTTP headers.
 *
 * \return				Returns TRUE if the headers are found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases the HTTP header at the given index specified by nIdx.
 *
 * \param nIdx			[in] An index of the header to remove.
 * \return				Returns TRUE if the header is found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases the HTTP header of which name equals to szName.
 * If the header has multiple values, you can specify the zero-based index for a specific value.
 *
 * \param szName		[in] A case-insensitive name of the header. NULL is not allowed.
 * \param nIdx			[in] A zero-based index for the multiple values of the header.
 * \return				Returns TRUE if the header is found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases all HTTP headers whose name equal to szName.
 *
 * \param szName		[in] A case-insensitive name of the header. NULL is not allowed.
 * \return				Returns TRUE if the headers are found and removed.
 */
...
/*!
 * This method adds a new HTTP header. The header's name and value can
 * not be NULL or empty string.
 *
 * \param szName		[in] A name of the header. NULL and empty string are not allowed.
 * \param szValue		[in] A value of the header. NULL and empty string are not allowed.
 * \throw				Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool, typename HttpEncoder>
void CHttpClientT<HttpTool, HttpEncoder>::AddHeader (PCSZ szName, PCSZ szValue)
	throw (Exception &)
{
	_ASSERTE ( (szName != NULL && szName[0] != '\0') ) ;
	_ASSERTE ( (szValue != NULL && szValue[0] != '\0') ) ;
	m_mapHeader.Add (szName, szValue) ;
}

/*!
 * This method modifies the value of the HTTP header of which name equals to szName.
 * If the header has multiple values, you can specify the zero-based index for a specific value.
 * If the header is not found and the index is zero, it will add a new header.
 * The header's name and value can not be NULL or empty string.
 *
 * \param szName		[in] A case-insensitive name of the header. NULL and empty string are not allowed.
 * \param szValue		[in] A value of the header. NULL and empty string are not allowed.
 * \param nIdx			[in] A zero-based index for the multiple values of the header.
 * \throw				Throws a httpclientexception if an error occurred.
 */
...
/*!
 * This method returns the name of the HTTP header at the given index specified by nIdx.
 ...
 *
 * \param nIdx			[in] A zero-based index.
...
/*!
 * This method returns the HTTP header at the given index.
 * If the index is out of range, it will return NULL. Otherwise it always returns a null-terminated string.
...
/*!
 * This method returns the HTTP header of which name equals to szName.
 * If the header has multiple values, you can specify the zero-based index for a specific value.
...
/*!
 * This method returns whether the HTTP header of which name equals to szName exists.
 * If the header has multiple values, you can specify the zero-based index for a specific value.
 *
 * \param szName		[in] A case-insensitive name of the header. NULL is not allowed.
 * \param nIdx			[in] A zero-based index for the multiple values of the header.
 * \return				Returns TRUE if the header exists. Otherwise returns FALSE.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method returns the number of the HTTP headers of which name equals to szName.
 * If the szName is NULL, it will return the number of headers.
 *
 * \param szName		[in] A case-insensitive name of the header. NULL is not allowed.
 * \return				The number of headers of which name equals to szName.
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * It is not allowed to call this method.
 *
 * \return				Returns TRUE if the parameters are found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases the HTTP parameter at the given index specified by nIdx.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 *
 * \param nIdx			[in] An index of the parameter to remove.
 * \return				Returns TRUE if the parameter is found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases the HTTP parameter of which name equals to szName.
 * If the parameter has multiple values, you can specify the zero-based index for a specific value.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
 * \param nIdx			[in] A zero-based index for the multiple values of the parameter.
 * \return				Returns TRUE if the parameter is found and removed.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method erases all HTTP parameters whose name equal to szName.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
...
/*!
 * This method adds a new HTTP parameter. You can specify the attribute of the parameter.
 * For more information about attributes, See the document of the ParamAttr enumerator.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 *
 * \param szName		[in] A name of the parameter. NULL is not allowed.
 * \param szValue		[in] A value of the parameter.
 * \param dwParamAttr	[in] An attribute of the parameter.
...
/*!
 * This method modifies the value of the HTTP parameter of which name equals to szName.
 * If the parameter has multiple values, you can specify the zero-based index for a specific value.
 * If the parameter is not found and the index is zero, it will add a new parameter.
 * You can specify the attribute of the parameter. For more information about attributes,
 * See the document of the ParamAttr enumerator.
 * If you call BeginPost or BeginUpload method and the POST operation is not finished,
 * It is not allowed to call this method.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
...
/*!
 * This method returns the name of the HTTP parameter at the given index specified by nIdx.
 * If the index is out of range, it will return NULL. Otherwise it always returns a null-terminated string.
 * The returned string is owned by the CHttpClient. So you can not free it.
 *
 * \param nIdx			[in] A zero-based index.
...
/*!
 * This method returns the HTTP parameter at the given index.
 * If the index is out of range, it will return NULL. Otherwise it always returns a null-terminated string.
 * The returned string is owned by the CHttpClient. So you can not free it.
 *
 * \param nIdx			[in] A zero-based index.
...
/*!
 * This method returns the HTTP parameter of which name equals to szName.
 * If the parameter has multiple values, you can specify the zero-based index for a specific value.
...
/*!
 * This method returns the attribute of the HTTP parameter at the given index.
 * If the nIdx is out of range or the value of the attribute is 0, it will return 0.
 *
 * \param nIdx			[in] A zero-based index.
...
/*!
 * This method returns the attribute of the HTTP parameter of which name equals to the szName.
 * If the parameter has multiple values, you can specify the zero-based index for a specific value.
 * If the nIdx is out of range or the value of the attribute is 0, it will return 0.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
...
/*!
 * This method returns whether the HTTP parameter of which name equals to szName exists.
 * If the parameter has multiple values, you can specify the zero-based index for a specific value.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
 * \param nIdx			[in] A zero-based index for the multiple values of the parameter.
 * \return				Returns TRUE if the parameter exists. Otherwise returns FALSE.
...
/*!
 * This method returns the number of HTTP parameters of which name equals to szName.
 * If the szName is NULL, it will return the number of parameters.
 *
 * \param szName		[in] A case-insensitive name of the parameter. NULL is not allowed.
 * \return				The number of parameters of which name equals to szName.
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method sets parameters for the ::InternetOpen function which is called by CHttpClient class.
 * All parameters passed to this method is forwarded to the ::InternetOpen function.
 * For more detailed information about the ::InternetOpen function, see the microsoft's SDK documentation.
 *
 ...
 * \param szProxyName		[in] A name of the proxy server which corresponds to the lpszProxyName parameter of the ::InternetOpen function.
...
/*!
 * This method returns the number of characters required to make a URL for HTTP 
 * GET request. If you want to pass some parameters to a web server using HTTP GET,
 * you have to append the parameters to the requested URL. This method calculates 
 ...
 * the calculated length. It does not check whether the URL is valid.
 * If the szUrl is NULL, it returns the length of the appended parameters.
 * The returned value does not include a terminating NULL character.
 *
 * \param szUrl		[in] An URL which is used to calculate the length of GET URL.
 * \return			The number of characters required. (Not including a terminating NULL character)
 * \throw			Throws a httpclientexception if an error occurred.
 */
template <typename HttpTool, typename HttpEncoder>
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::MakeGetUrlLen") ;
	}
...
/*!
 * This method appends the HTTP parameters to the input URL for HTTP GET request.
 * It uses the HTTP parameters which is saved in the CHttpClient.
 * It does not check whether the URL is valid.
 * If the szUrl is NULL, it copies only the parameters.
 *
 * \param szBuff	[out] A buffer to save the GET URL. The buffer can not be NULL.
 * \param szUrl		[in] An URL which is used to make a URL for HTTP GET request.
	...
	throw (Exception &)
{
	return HttpTool::OpenInternet (
		m_szInternetUserAgent ? m_szInternetUserAgent : HttpTool::szDefUsrAgent
		, m_dwInternetAccessType, m_szInternetProxyName, m_szInternetProxyBypass, m_dwInternetFlags) ;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::OpenConnection") ;
	}

	PSZ				szServerAddr = (PSZ) ::malloc (cbAddress) ;
...
/*!
 * This method sets a user name and password which are used to access HTTP proxy server.
	...
	throw (Exception &)
{
	...
	_ASSERTE ( szUserName && HttpTool::StringLen (szUserName) ) ;
	_ASSERTE ( szPassword && HttpTool::StringLen (szPassword) ) ;
...
/*!
 * This method returns the current proxy user name.
 * It always returns a null-terminated string. The returned string is owned by
 * the CHttpClientT. So you can not free it.
 *
 * \return				The current proxy user name.
 */
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::PCSZ
	...
	throw (Exception &)
{
	...
	DWORD		cchProxyUserName;
	DWORD		cchProxyPassword;
	...
	try {
		...
		throw ("Error en CHttpClientT<HttpTool, HttpEncoder>::ApplyProxyAccount") ;
	}
	...
	if ( objAnalyzer.PathLen () == 0 ) {
		...
		if ( cchAfterPath == 0 )
			...
			try {
				...
				HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::OpenRequest") ;
			}

			szUrlPath = (PCSZ) ::malloc (cbAfterPath) ;
			if ( szUrlPath == NULL )
				HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
 ...
 * If an exception occurred, it is possible that the headers are added partially.
 *
	...
	try {
		...
		if ( !m_mapParam.Empty () ) {
			...
			try {
				...
				HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_RequestGetEx") ;
			}

			if ( NULL == (szGetUrl = (PCSZ) ::malloc (cbGetUrl)) )
				HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
 ...
 * information of the POST request using Query method.
 ...
 * \throw				Throws a httpclientexception if an error occurred.
 *
 * \sa		OpenInternet, OpenConnection, ::InternetOpen, ::InternetConnect, ::HttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurred.
 *
 * \sa		OpenInternet, ::InternetOpen, ::InternetConnect, ::HttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurred.
 *
 * \sa		::InternetConnect, ::HttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method starts a new HTTP UPLOAD request. Actually the HTTP UPLOAD request does not exist.
 * This method starts a new HTTP POST request with another content-type (multipart/form-data).
 * If you call this method, you can retrieve progress information of the POST request using Query method.
 * To finish the request, you have to call the Proceed method.
 * If the szUrl does not start with "https://", "http" is used as the protocol.
 * If the caller passes the hInternet parameter which is returned by ::InternetOpen,
 * the CHttpClient uses the passed handle instead of creating a new one.
 ...
 * If you starts a POST context using BeginPost or BeginUpload method,
 * you can retrieve progress information using this method.
 * If an exception occurred, the current POST context is automatically canceled.
 *
 * \param objPostStat	[out] A CHttpPostStatT object.
 *
 * \sa		CHttpPostStatT
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * BeginPost or BeginUpload method.
 *
 ...
 * \return				if all bytes are transmitted, returns a pointer to
 *						a CHttpResponseT object. Otherwise returns NULL.
 * \throw				Throws a httpclientexception if an error occurred.
 *
 * \sa		CHttpResponseT
 */
	...
	throw (Exception &)
{
	...
	while ( !(pResponse = Proceed (1024 * 128)) ) ;
	return pResponse ;
}
 ...
 * Actually the HTTP UPLOAD request does not exist. This method retrieves the resource
 * specified by the szUrl using HTTP POST request with another content-type (multipart/form-data).
 * If the szUrl does not start with "https://", "http" is used as the protocol.
 * If the caller passes the hInternet parameter which is returned by ::InternetOpen,
 * the CHttpClient uses the passed handle instead of creating a new one.
 * The caller is responsible for closing the hInternet handle when it has finished with it.
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_WritePost") ;
	}

	_WritePost (reinterpret_cast<const BYTE *> (szValue), cbValue, bIsValue) ;
}
	...
	throw (Exception &)
{
	...
	try {
		...
		HttpTool::SendRequestEx (hRequest, dwTotalByte);

		// Starts a new POST state
		m_objPostStat._MakeActive (nTotalByte, nValuesTotalByte, m_mapParam.Count (), 0) ;
	...
	throw (Exception &)
{
	...
	_ASSERTE ( m_hInternet != NULL ) ;
	_ASSERTE ( m_hConnection != NULL ) ;
	_ASSERTE ( m_hRequest != NULL ) ;
	_ASSERTE ( nDesired != 0 ) ;
			...
			// Writes the parameter name
			{
				...
				try {
					if ( (iter->second).dwFlag & ParamEncodedName ) {
						...
						size_t	cbRequired = _UrlEncodeLenA (iter->first) ;

						if ( cbRequired > 0 ) {
							cbRequired += 1 ;
							cbRequired *= sizeof (CHAR) ;

							if ( NULL == (szNameA = (PSTR) ::malloc (cbRequired)) )
								HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
				...
				if ( (iter->second).dwFlag & ParamEncodedValue ) {
					...
					if ( cbValue > 0 ) {
						...
						try {
							...
							HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_ProceedPostContext") ;
						}

						if ( NULL == (szValueA = static_cast<PSTR> (::malloc (cbRequired))) )
							HttpTool::ThrowException (HTTPCLIENT_ERR_OUT_OF_MEMORY) ;
						bNeedToFreeValueA = TRUE ;
	...
	throw (Exception &)
{
	...
	try {
		...
		if ( !m_mapParam.Empty () ) {
			try {
				...
				if ( nPostedFileCount ) {
					...
					ahFileHandles = (HANDLE *) ::calloc (nPostedFileCount, sizeof (HANDLE)) ;
					...
					aszMimeTypes = (LPSTR *) ::calloc (nPostedFileCount, sizeof (LPSTR)) ;
				...
				*/
				...
				HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_StartUploadContext 0") ;
			}
		...
		try {
			...
			HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_StartUploadContext 1") ;
		}
		...
		m_objPostStat._MakeActive (nTotalByte, nValuesTotalByte, m_mapParam.Count (), nPostedFileCount) ;
		m_bBorrowedInternet = bBorrowedInternet ;
		m_hInternet = hInternet ;
		m_bBorrowedConnection = bBorrowedConnection ;
		m_hConnection = hConnection ;
	...
	throw (Exception &)
{
	...
	try {
		...
		if ( m_objPostStat.CurrParamIsComplete () ) {
			...
			// Starts a new parameter
			m_objPostStat._StartNewEntry (iter->first, nEntryValueTotalByte
		...
		_EndPostContext () ;			// Aborts the POST context if an error occurred
		throw ;
	}
	catch (...) {
		_EndPostContext () ;			// Aborts the POST context if an error occurred
		HttpTool::ThrowException ("Error en CHttpClientT<HttpTool, HttpEncoder>::_ProceedUploadContext") ;
	} 
	return NULL ;
}
...
CHttpClientT<HttpTool, HttpEncoder>::_ReleasePostResponse ()
	throw (Exception &)
{
	_ASSERTE ( m_objPostStat.IsActive () ) ;
